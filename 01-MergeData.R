...
# Set data directories and load libraries
...
library(ggfortify); library(pals); library(igraph); library(reshape2)
...
# Remove outliers in tracking data from all datasets
...
# Run Facetnet
alpha <- 0.9 # This needs to be set as something arbitrary but the value has no effect since t_steps is set to 0
m <- 2
t_steps <- 1
...
# Merge Facetnet data with other metadata
...
# Establish social maturity from Facetnet output
